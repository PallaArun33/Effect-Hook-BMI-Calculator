...
const BmiCalculator = () => {
  const getStoredHeight = JSON.parse(localStorage.getItem('height'))
  const getStoredWeight = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(
    getStoredHeight !== null ? getStoredHeight : 170,
  )
  const [weight, setWeight] = useState(
    getStoredWeight !== null ? getStoredWeight : 60,
  )
